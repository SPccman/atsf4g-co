...
int task_action_player_login::operator()(hello::message_container& msg) {
    ...
    if (!user || !user->is_inited()) {
        ...
        if (hello::err::EN_DB_RECORD_NOT_FOUND == res) {
            ...
            if (res < 0) {
                WLOGERROR("save player data for %s failed, msg:\n%s", user->get_open_id().c_str(),
                          tbu.DebugString().c_str());
                set_rsp_code(hello::EN_ERR_SYSTEM);
                return res;
            }
...
int task_action_player_login::on_failed() {
    ...
    std::shared_ptr<session> s = get_session();
    if (!s) {
        WLOGERROR("session [0x%llx,0x%llx] not found",
    ...
    WLOGERROR("session [0x%llx,0x%llx] login failed, rsp code: %d, ret code: %d",
          static_cast<unsigned long long>(get_gateway_info().first),
          static_cast<unsigned long long>(get_gateway_info().second),
              get_rsp_code(),
              get_ret_code()
