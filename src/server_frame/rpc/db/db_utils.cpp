...
namespace rpc {
    namespace db {
        ...
        bool redis_args::empty() const {
            return 0 == used_;
        }

        size_t redis_args::size() const {
            return used_;
        }

        const char** redis_args::get_args_values() {
            return segment_value_.data();
        }

        const size_t* redis_args::get_args_lengths() const {
            return segment_length_.data();
        }
        ...
        bool redis_args::push(const std::string& str) {
        ...
        int unpack_message(::google::protobuf::Message& msg, const redisReply* reply, std::string* version) {
            ...
            for(size_t i = 0; i < reply->elements - 1; i += 2) {
                ...
                if (NULL == fd) {
                    //has_failed = true;
                    WLOGERROR("unpack message %s failed, parse segment name %s of table user failed, maybe",
...
#define CASE_REDIS_DATA_TO_PB_INT(pbtype, cpptype, func) \
                    case pbtype: { \
                        ...
                        if (REDIS_REPLY_INTEGER == value->type) { \
                            ...
                            WLOGERROR("unpack message %s failed, type of %s in pb is a message, but the redis reply type is not string nor integer(reply type=%d).", msg.GetDescriptor()->full_name().c_str(), key->str, value->type); \
                            has_failed = true; \
                        } \
                        break; \
                    }\
                ...
                switch(fd->cpp_type()) {
                    ...
                    case google::protobuf::FieldDescriptor::CPPTYPE_MESSAGE : {
                        ...
                        if (REDIS_REPLY_STRING != value->type || NULL == value->str) {
                            ...
                            if (NULL == data_msg) {
                                has_failed = true;
                                WLOGERROR("mutable message name [%s] of table user failed", key->str);
                                continue;
                            }
        ...
        int pack_message(const ::google::protobuf::Message& msg, redis_args& args,
            ...
            for (size_t i = 0; i < fds.size(); ++ i) {
                ...
                if (NULL == data_allocated) {
                    WLOGERROR("pack message %s failed, alloc %s key failed",
                              msg.GetDescriptor()->full_name().c_str(),
                              fds[i]->name().c_str()
                    );
                    return hello::err::EN_SYS_MALLOC;
...
#define CASE_PB_INT_TO_REDIS_DATA(pbtype, cpptype, cppformat, func) \
                ...
                switch(fds[i]->cpp_type()) {
                    ...
                    default: {
                        WLOGERROR("message %s field %s(type=%s) invalid",
                                  msg.GetDescriptor()->full_name().c_str(),
