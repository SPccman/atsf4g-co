...
int32_t db_msg_dispatcher::init() UTIL_CONFIG_OVERRIDE {
    ...
    if (NULL == tick_timer_) {
        ...
        do {
            ...
            res = uv_timer_start(tick_timer_, db_msg_dispatcher::on_timer_proc,
                                 logic_config::me()->get_cfg_db().proc,
                                 logic_config::me()->get_cfg_db().proc);
            if (0 != res) {
...
int db_msg_dispatcher::send_msg(channel_t::type t, const char* ks, size_t kl,
...
void db_msg_dispatcher::log_debug_fn(const char* content) {
    WCLOGDEBUG(log_categorize_t::DB, "%s", content);
}

void db_msg_dispatcher::log_info_fn(const char* content) {
    WCLOGINFO(log_categorize_t::DB, "%s", content);
}
...
int db_msg_dispatcher::script_load(redisAsyncContext *c, uint32_t type) {
    ...
    open_file(logic_config::me()->get_cfg_db().db_script_file[type % hello::EnDBScriptShaType_ARRAYSIZE].c_str(), script);
...
void db_msg_dispatcher::cluster_on_connected(hiredis::happ::cluster* clu, hiredis::happ::connection* conn, const struct redisAsyncContext*, int status) {
    ...
    // 注入login脚本和user脚本
    me()->script_load(conn->get_context(), hello::EN_DBSST_LOGIN);
    me()->script_load(conn->get_context(), hello::EN_DBSST_PLAYER);
...
int db_msg_dispatcher::cluster_send_msg(hiredis::happ::cluster& clu, const char* ks, size_t kl,
    uint64_t task_id, uint64_t pd,
    unpack_fn_t fn, int argc, const char **argv, const size_t *argvlen) {
