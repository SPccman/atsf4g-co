...
int32_t ss_msg_dispatcher::send_to_proc(uint64_t bus_id, msg_ptr_t msg) {
    ...
    {
        WLOGERROR("send to proc [0x%llx] failed: require %llu, only have %llu",
    ...
    WLOGDEBUG("send msg to gateway [0x%llx] %llu bytes\n%s",
      static_cast<unsigned long long>(bus_id),
      static_cast<unsigned long long>(msg_buf_len),
...
int32_t ss_msg_dispatcher::send_to_proc(uint64_t bus_id, const void* msg_buf, size_t msg_len) {
    ...
    if (res < 0) {
        WLOGERROR("send msg to proc [0x%llx] %llu bytes failed, res: %d",
          static_cast<unsigned long long>(bus_id),
          static_cast<unsigned long long>(msg_len),
          res
        );
    } else {
        WLOGDEBUG("send msg to proc [0x%llx] %llu bytes success",
          static_cast<unsigned long long>(bus_id),
          static_cast<unsigned long long>(msg_len)
        );
    }
