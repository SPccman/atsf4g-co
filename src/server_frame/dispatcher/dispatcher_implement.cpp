...
int32_t dispatcher_implement::on_recv_msg(msg_ptr_t msgc, const void* msg_buf, size_t msg_size) {
    ...
    if (NULL != WDTLOGGETCAT(util::log::LogWrapper::categorize_t::DEFAULT) &&
        WDTLOGGETCAT(util::log::LogWrapper::categorize_t::DEFAULT)->check(util::log::LogWrapper::level_t::LOG_LW_DEBUG)) {
    ...
    if (task_id > 0) { // 如果是恢复任务则尝试切回协程任务
        ...
        // TODO dst_id
        msgc->mutable_src_server()->set_dst_id(task_id);
