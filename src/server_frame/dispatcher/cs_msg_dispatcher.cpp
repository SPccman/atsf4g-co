    ...
    switch (req_msg.head.cmd) {
        case ATFRAME_GW_CMD_POST: {
            ...
            if (!session_manager::me()->find(session_key)) {
                WLOGERROR("session [0x%llx, 0x%llx] not found, try to kickoff",
                  static_cast<unsigned long long>(session_key.bus_id),
                  static_cast<unsigned long long>(session_key.session_id)
                );
                ret = hello::err::EN_SYS_NOTFOUND;

                send_kickoff(session_key.bus_id, session_key.session_id, ::atframe::gateway::close_reason_t::EN_CRT_RESET);
                break;
            }
        ...
        case ATFRAME_GW_CMD_SESSION_ADD: {
            ...
            WLOGINFO("create new session [0x%llx, 0x%llx], address: %s:%d",
                     static_cast<unsigned long long>(session_key.bus_id),
                     static_cast<unsigned long long>(session_key.session_id),
        ...
        case ATFRAME_GW_CMD_SESSION_REMOVE: {
            ...
            WLOGINFO("remove session [0x%llx, 0x%llx]",
                     static_cast<unsigned long long>(session_key.bus_id),
                     static_cast<unsigned long long>(session_key.session_id)
            );
...
int32_t cs_msg_dispatcher::send_data(uint64_t bus_id, uint64_t session_id, const void* buffer, size_t len) {
    ...
    if (NULL == msg.body.post) {
        if (0 == session_id) {
            WLOGERROR("broadcast data to atgateway 0x%llx failed when malloc post",
                      static_cast<unsigned long long>(bus_id)
            );
        } else {
            WLOGERROR("send data to session [0x%llx, 0x%llx] failed when malloc post",
                      static_cast<unsigned long long>(bus_id),
    ...
    if (ret < 0) {
        if (0 == session_id) {
            WLOGERROR("broadcast data to atgateway 0x%llx failed, res: %d",
                      static_cast<unsigned long long>(bus_id),
                      ret
            );
        } else {
            WLOGERROR("send data to session [0x%llx, 0x%llx] failed, res: %d",
                      static_cast<unsigned long long>(bus_id),
                      static_cast<unsigned long long>(session_id),
                      ret
            );
