...
std::list<task_action_cs_req_base::msg_type>& task_action_cs_req_base::get_rsp_list() {
    return rsp_msgs_;
}
...
void task_action_cs_req_base::send_rsp_msg() {
    ...
    for(std::list<msg_type>::iterator iter = rsp_msgs_.begin(); iter != rsp_msgs_.end(); ++iter) {
        ...
        if (res) {
            player::ptr_t owner_player = get_player();
            if(owner_player) {
                WLOGERROR("task %s [0x%llx] send message to player %s failed, res: %d", name(), get_task_id_llu(),
                  owner_player->get_open_id().c_str(), res
                );
            } else {
                WLOGERROR("task %s [0x%llx] send message to session [0x%llx,0x%llx] failed, res: %d", name(), get_task_id_llu(),
                  static_cast<unsigned long long>(sess->get_key().bus_id),
                  static_cast<unsigned long long>(sess->get_key().session_id), res
                );
            }
        }
    }
